...
void setup() {
  Serial.begin(115200);
  Ps3.begin("a0:5a:5a:a0:11:d8");  // configurate MAC address of Joystick and ESP32 (Install SixaxisPairTool for equipment mapping)
  ...
  myservo.write(105);                     // setup servo motor to go to position (0-180 degree) in variable 'pos' (pos=90(Stop), 90 < Pos < 180 (turn right), Pos< 90 (Turn left))
  time_start = millis();
}
  ...
  if (Ps3.isConnected() && millis() - lastSend > 50) {  
    ...
    if (Ps3.data.button.up) {  // Push Pad UP, motor move forward
      ...
      if (!Ps3.data.button.square && !Ps3.data.button.circle) {
        myservo.write(105);  // setup servo motor stop position in variable 'pos'
      }
    }
    ...
    if (Ps3.data.button.circle) {  // Push Pad Right, motor turn right
      myservo.write(140);          // setup servo motor moving to position in variable 'pos' and turn right 30 degree from 90 degree axis
    }
    if (Ps3.data.button.start) {  // Push Pad Start as AI mode
      ...
      while (true) {
        ...
        display.println("O to start");
        display.setCursor(0, 20);
        display.println("select to manual");
        ...
        if (start == true) {
          while (true) {
            ...
            float lux = lux_light();
            // Serial.println(lux);
            Slope slope = imu_balancing();
            bool uphill = slope.uphill;
            bool downhill = slope.downhill;
